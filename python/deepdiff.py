...
class DeepDiff(object):
    ...
    **Supported data types**
    ...
    def __diff_obj(self, t1, t2, parent, parents_ids=frozenset({})):
        ...
        try:
            ...
            try:
                ...
                self.unprocessed.append({'key':parent, 'old':t1, 'new':t2})
                return
    ...
    def __diff_iterable(self, t1, t2, parent=[], parents_ids=frozenset({})):
        ...
        for i, (x, y) in enumerate(_ver.zip_longest(t1, t2, fillvalue=ListItemRemovedOrAdded)):
    ...
    def __diff(self, t1, t2, parent=[], parents_ids=frozenset({})):
        ...
        if type(t1) != type(t2):
            self.type.append({'key':parent, 'old':self.__gettype(t1), 'new':self.__gettype(t2)})

        elif isinstance(t1, _ver.basestring):
            if t1 != t2:
                self.value.append({'key':parent, 'old':t1, 'new':t2})

def ECEcalib(sig, ecechannel, timestamp, freq=None, calibdata=None):
    from mdsplus import Ident, Float32, StringArray
    ...
    # construct output
    if isinstance(freq,Ident):
        Float32(freqGHz).setUnits('GHz').setTdiVar(freq.name)
    if isinstance(calibdata,Ident):
        StringArray([conf_file,cali_file,para_file]).setUnits('Ghz').setTdiVar(calibdata.name)
